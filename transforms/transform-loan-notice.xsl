...
<xsl:template name="section-01">
    ...
    <xsl:call-template name="generic-line-mit">
       <xsl:with-param name="line" select="'The item(s) listed below will be due on the date indicated.  Please remember to return this material to any MIT library on or before the due date.'"/>
    </xsl:call-template>
    <xsl:call-template name="blank-line"/>
</xsl:template>
