...
<xsl:template name="section-01">
    ...
    <xsl:call-template name="generic-line-mit">
       <xsl:with-param name="line" select="'Due to the renovation of Dewey library, all Dewey Library materials on 30-day loan are being changed automatically to a new extended loan, with a due date of March 2, 2009.'"/>
    </xsl:call-template>
    <xsl:call-template name="generic-line-mit">
       <xsl:with-param name="line" select="'Please note:'"/>
    </xsl:call-template>
    <xsl:call-template name="generic-line-mit">
       <xsl:with-param name="line" select="'The recall policy continues to apply during this period. If books are recalled, they need to be returned promptly by the due date indicated in the recall notice.'"/>
    </xsl:call-template>
    <xsl:call-template name="generic-line-mit">
       <xsl:with-param name="line" select="'The item(s) listed below have changed to this new due date.  Please remember to return this material to any MIT library on or before the due date.'"/>
    </xsl:call-template>
    <xsl:call-template name="blank-line"/>
</xsl:template>
